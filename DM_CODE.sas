...
data DM.DM1;
	...
	else if count = 1 then RACE = propcase(strip(value));
	else RACE = "";
...
data DM.DM2;
	...
	IPSTDT_CHAR = tranwrd(strip(IPSTDT_RAW),'/','');
...
data DM.RFENDDTC;
	...
	RFENDTC_NUM = input(tranwrd(strip(EOSTDT_RAW), '/', ''), date11.);;
...
data DM.enrl_dates;
    ...
    do i=1 to dim(dts);
        if not missing(dts{i}) then do;
            dt_num = input(tranwrd(strip(dts{i}), '/', ''), date11.);
            output;
        end;
    end;
...
proc sort data = rawdata.box; by KITID; run;PROC PRINT;RUN;
proc sort data = DM.BOXIDS; by IPBOXID; run;PROC PRINT; RUN;
...
data DM.SIX;
	...
	drop RFICDTC_CHAR RFICDTC_NUM;
proc print;

/*Else assign the value of RFICDTC for RFSTDTC*/
